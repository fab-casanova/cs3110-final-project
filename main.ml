    ...
    if input = "q" then ()
      ...
      if List.mem prop props then (
        ...
        ANSITerminal.print_string [ ANSITerminal.blue ]
         ...
         ^ " will cost you $" ^ string_of_int unmort_cost
         ^ ".\n\
...
let rec barter_respond player asked_player prop price game wants_to_buy = ()

let rec propose_price player asked_player prop game wants_to_buy =
  ANSITerminal.print_string [ ANSITerminal.blue ]
  ...
  try
    ...
    if desired < 0 then (
      ANSITerminal.print_string [ ANSITerminal.red ]
        "Please only enter a nonnegative number\n";
      propose_price player asked_player prop game wants_to_buy)
    else (
      ANSITerminal.print_string [ ANSITerminal.blue ]
        ("Would you like to "
        ^
        if wants_to_buy then "buy "
        else
          "sell " ^ prop_name prop ^ " for " ^ input
          ^ "?\nEnter 'y' to confirm, anything else to choose another price\n");
      match read_line () with
      | "y" -> barter_respond player asked_player prop desired game wants_to_buy
      | _ -> propose_price player asked_player prop game wants_to_buy)
  with _ ->
    ANSITerminal.print_string [ ANSITerminal.red ]
      "Please only enter a nonnegative number\n";
    propose_price player asked_player prop game wants_to_buy
