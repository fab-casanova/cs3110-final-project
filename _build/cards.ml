...
let advance_to_property property_name game =
  ...
  if property_name = "Pass Go" then get_money 200 game else []
...
let pay_all_players payment_per_player game : transaction list =
  ...
  let rec trans_list current players acc =
    match players with
    | [] -> acc
    | h :: t ->
        if h <> current then
          if payment_per_player > 0 then
            trans_list current t
              ((Some current, payment_per_player, Some h) :: acc)
          else
            trans_list current t
              ((Some h, -payment_per_player, Some current) :: acc)
        else trans_list current t acc
  in
  trans_list current (get_players game) []
